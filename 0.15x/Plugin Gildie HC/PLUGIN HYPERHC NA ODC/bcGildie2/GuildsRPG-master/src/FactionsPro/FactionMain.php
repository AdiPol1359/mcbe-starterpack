...
class FactionMain extends PluginBase implements Listener {
    ...
    public function onEnable() {
        ...
        $this->prefs = new Config($this->getDataFolder() . "GuildsOptions.yml", CONFIG::YAML, array(
            ...
            "PowerGainedPerKillingAnEnemy" => 100,
    		"PowerReducedPerDeathByAnEnemy" => 10,
            "PowerGainedPerAlly" => 0,
            "AllyLimitPerFaction" => 5,
            "TheDefaultPowerEveryFactionStartsWith" => 1000,
            "###Economys###",
            "CreateCost" => 0,
		    "ClaimCost" => 0,
	    	"OverClaimCost" => 0,
	    	...
	    	"SetHomeCost" => 0,
          "###GuildsMoneys###",
          "GuildsMoneyGainPerKill" => 0,
          "GuildsMoneyLostPerDeath" => 0,
        ));
    ...
    public function getAllAllies($s, $faction) {
        ...
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            ...
            $team .= TextFormat::DARK_GRAY . TextFormat::DARK_GRAY . " || §7Gildia:§c " . $row[$i]['faction2'] . " §8|| " . TextFormat::RESET;
            $i = $i + 1;
        }

        $s->sendMessage("§o§7|| §8[ §cSojusze: $faction §8] §7||", true);
        $s->sendMessage($team);
    }
    ...
    public function sendListOfTop10FactionsTo($s) {
        ...
        $row = array();
        $i = 0;
        $s->sendMessage("§8[§7---------- §8[§aTOP GILDIE§8]§7 ----------§8]");
        while ($resultArr = $result->fetchArray(SQLITE3_ASSOC)) {
            $j = $i + 1;
            $cf = $resultArr['faction'];
            $pf = $this->getFactionPower($cf);
            $df = $this->getNumberOfPlayers($cf);
            $s->sendMessage(TextFormat::BLUE . TextFormat::GOLD ."§a" . $j . "§7. " . TextFormat::GREEN . "§7Gildia:§a $cf" . TextFormat::GOLD . " §7- " . TextFormat::GREEN . " $pf §7Punktow" . TextFormat::GOLD . "" . TextFormat::LIGHT_PURPLE . "" . TextFormat::RESET);
            $i = $i + 1;
        }
    }
        ...
        if ($this->cornerIsInPlot($x + $arm, $z + $arm, $x - $arm, $z - $arm)) {
            ...
            if ($this->prefs->get("EnableOverClaim")) {
                if ($power_sender < $power_claimedBy) {
                    $sender->sendMessage($this->formatMessage("Ten teren jest zajęty!"));
                } else {
                    $sender->sendMessage($this->formatMessage("Ten teren jest zajęty!"));
                }
                return false;
	...
	public function formatMessage($string, $confirm = false) {
		if($confirm) {
			return "§f• §8[" . TextFormat::GREEN . "§aGildie" . TextFormat::WHITE . "§8] " . TextFormat::GRAY . "$string §f•";
		} else {	
			return "§f• §8[" . TextFormat::GREEN . "§aGildie" . TextFormat::WHITE . "§8] " . TextFormat::GRAY . "$string §f•";
		}
	}
