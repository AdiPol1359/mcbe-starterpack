...
class Main extends PluginBase implements CommandExecutor,Listener {
	...
	private function cmdHelp(CommandSender $sender,$args) {
		$cmds = [
			"staty" => ["[player ...]",mc::_("Show player scores")],
			"topka" => ["[online]",mc::_("Show top players")],
		];
		if (count($args)) {
			foreach ($args as $c) {
				if (isset($cmds[$c])) {
	...
	private function cmdTops(CommandSender $c,$args) {
		...
		$c->sendMessage(mc::_("§c• Topka zabójstw •"));
		$i = 1;
		foreach ($res as $r) {
	...
	private function cmdStats(CommandSender $c,$args) {
		...
		foreach ($args as $pl) {
			...
			if ($score == null) {
				$c->sendMessage(mc::_("§c• Nie znałeziono nic z bazy danych dla %1% •",$pl));
				continue;
			} else {
