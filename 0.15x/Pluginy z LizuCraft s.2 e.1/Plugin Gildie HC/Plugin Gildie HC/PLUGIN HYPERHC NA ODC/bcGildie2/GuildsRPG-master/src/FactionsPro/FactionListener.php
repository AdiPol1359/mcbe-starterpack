...
class FactionListener implements Listener {
	...
	public function factionChat(PlayerChatEvent $PCE) {
		...
		if($this->plugin->motdWaiting($player)) {
			if(time() - $this->plugin->getMOTDTime($player) > 30) {
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("Czas minął."));
				...
				$PCE->setCancelled(true);
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("Zaktualizowano opis gildi...", true));
			}
			return true;
		}
	...
	public function factionBlockBreakProtect(BlockBreakEvent $event) {
		if($this->plugin->isInPlot($event->getPlayer())) {
			if($this->plugin->inOwnPlot($event->getPlayer())) {
				...
				$event->getPlayer()->sendPopup("§cNie możesz niszczyć tutaj blokow. Ten teren jest zajęty!");
				return true;
			}
		}
	}
	...
	public function factionBlockPlaceProtect(BlockPlaceEvent $event) {
		if($this->plugin->isInPlot($event->getPlayer())) {
			if($this->plugin->inOwnPlot($event->getPlayer())) {
				...
				$event->setCancelled(true);
				$event->getPlayer()->sendPopup("§cNie możesz stawiać tutaj blokow. Ten teren jest zajęty!");
				return true;
			}
		}
	}
  ...
  $killer->sendPopup("§7Dodano§c ".$e." §7punktów gildi!");
  $killer->sendTip("");
  ...
  $ent->sendPopup("§7Stracono§c ".$e." §7punktów gildi!");
  $ent->sendTip("");
	...
	public function onPlayerJoin(PlayerJoinEvent $event) {
		...
		public function Ruch(PlayerMoveEvent $event){
		$gracz = $event->getPlayer();
        if($gracz instanceof Player) {
	    if($this->plugin->isInPlot($gracz) === true){
	    if($this->plugin->inOwnPlot($gracz) === true){
	     $this->plugin->getPlayerFaction($gracz);
		 $efekt2 = Effect::getEffect(3);
         $efekt2->setDuration(200);
         $gracz->addEffect($efekt2);
            }
			}
		}
}
	public function interaktmadafaka(PlayerInteractEvent $event) {
		if($this->plugin->isInPlot($event->getPlayer())) {
			if($this->plugin->inOwnPlot($event->getPlayer())) {
				...
				if($event->getPlayer()->getItemInHand()->getId() === 325) {
				$event->setCancelled(true);
				$event->getPlayer()->sendPopup("§cNie możesz stawiać tutaj blokow. Ten teren jest zajęty!");
				return true;
			}
